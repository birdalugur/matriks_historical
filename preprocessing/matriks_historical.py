...
class MatriksData(object):
    def __init__(self, username, password):
        ...
        self.__token_test = self.__login(
            self.__username, self.__password, True)
        self.__auth_header = {'Authorize': 'jwt ' + self.__token}
        print(self.__token)
        if self.__token:
            self.__disco = requests.get(
                'https://api.matriksdata.com/disco.json')
        if self.__token_test:
            self.__disco_test = requests.get(
                'https://apitest.matriksdata.com/disco.json')
    ...
    def __login(self, username, password, is_test):
        ...
        if is_test:
            r = requests.get("http://api.matriksdata.com/login",
                             headers=extra_headers, auth=(username, password))
        else:
            r = requests.get("http://apitest.matriksdata.com/login",
                             headers=extra_headers, auth=(username, password))
    ...
    def requester(self, url, **kwargs):
        ...
        for i in range(0, len(date_range) - 1):
            ...
            request = self.__request_builder(
                url, start=start, end=end, **kwargs)
            print(request)
            response = None
            ...
            try:
                response_data = json.loads(
                    response.text, object_pairs_hook=OrderedDict)
    ...
    def trade_bs(self, symbol, start_date, end_date):
        return self.requester('https://api.matriksdata.com/dumrul/v1/tick/trade_bs', symbol=symbol,
